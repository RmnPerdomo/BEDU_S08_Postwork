...
# ¿Cómo influyen el nivel socioeconómico, recursos financieros y seguridad
# alimentaria en el gasto de alimentos saludables y no saludables en los
# hogares de la población en México?

"2) Realiza un análisis descriptivo de la información"

# El dataframe con valores factorizados
df.fact <- df
...
# Revisión de variables categóricas como factores
summary(df.fact)


# Frecuencias para nse5f
barplot(table(df.fact$nse5f))
barplot(table(df.fact$refin))
barplot(table(df.fact$sexojef))
barplot(table(df.fact$IA))

# Gŕaficas de cajas de gastos en alimentos saludables y no saludables
# contra nivel socioeconómico
boxplot(ln_als ~ nse5f, data = df.fact)
boxplot(ln_alns ~ nse5f, data = df.fact)
